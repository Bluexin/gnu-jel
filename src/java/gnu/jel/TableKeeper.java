...
public class TableKeeper {
  ...
  static {
    ...
    try {
      ...
      String runtimeJelPackage = c.getCanonicalName().substring(0, c.getCanonicalName().lastIndexOf('.'));
      for(int i=10; i<specialTypesStr.length;i++)
        specialClasses[i]=specialTypes[i]=Class.forName(specialTypesStr[i].replace("gnu.jel", runtimeJelPackage));

      for(int i=0;i<8;i++)
        specialClasses[i]=specialTypes[i]=(Class<?>)
          Class.forName(specialTypesStr[i+20].replace("gnu.jel", runtimeJelPackage)).getField("TYPE").get(null);
      ...
      for (int i=0; i<specialClassesAddStr.length;i++)
        specialClasses[specialTypesStr.length+i]=
            Class.forName(specialClassesAddStr[i].replace("gnu.jel", runtimeJelPackage));
